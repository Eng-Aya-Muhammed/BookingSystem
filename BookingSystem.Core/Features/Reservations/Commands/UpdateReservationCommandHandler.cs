...
namespace BookingSystem.Core.Features.Reservations.Commands
{
    public class UpdateReservationCommandHandler(
     ...
     IUnitOfWork uow,
     IMapper mapper
 ) : IRequestHandler<UpdateReservationWithIdCommand, ReservationDto>
    {
        public async Task<ReservationDto> Handle(UpdateReservationWithIdCommand request, CancellationToken cancellationToken)
        {
            var reservation = await repository.GetByIdAsync(request.Id)
                              ?? throw new KeyNotFoundException("Reservation not found");

            mapper.Map(request.Command, reservation);
